...
@can('Gestion_User')
    <h1 class="text-white text-center font-bold bg-gray-100 p-2 rounded" style="min-width: 100px!important;position: absolute;margin-top: 80px;margin-left: 30px;color:chartreuse">Utilisateurs</h1>

    <br>
<br>
<br>

<div class='flex flex-col overflow-x-auto overflow-y-auto p-8' style="margin-top: 100px!important">
    @if (session('success'))
        <div class="alert">
            L'opération a été realiser avec success
    ...
    <table id="myTable" style="background-color: white!important; border-radius:20px" class='table-auto w-full '>
      <thead >
        <tr style="background-color: chartreuse" class='text-white font-light'>
            <th class='px-4 py-2 text-center'>Photo Profile</th>
        ...
        <tr>
          ...
          <td class='px-4 py-2 text-center'>{{$item->role->name ?? 'No Role Assigned'}}</td>
           <td class='px-4 py-2 text-center'>
               <div class="flex">
                   <div class="w-1/2 ">
        <form id="delete-form-{{ $item->id }}" action="{{ route('users.destroy', ['user'=>$item->id])}}" method="POST">
          @csrf
          @method("DELETE")
          <button type="submit">
            <svg class="text-white" xmlns="http://www.w3.org/2000/svg" height="20" width="20" viewBox="0 0 448 512">
