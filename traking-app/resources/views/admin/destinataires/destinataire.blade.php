...
@can('afficher_client')
    <h1 class="text-white text-center font-bold bg-gray-100 p-2 rounded" style="min-width: 100px!important;position: absolute;margin-top: 80px;margin-left: 30px;color:darkred">Clients</h1>
    ...
    <div class='flex flex-col overflow-x-auto overflow-y-auto p-8' style="margin-top: 80px!important">
        ...
        @endif
<form action="{{route('export_destinataire')}}" method="get">
...
<div class="flex">
            @can('supprimer_client')
                <div class="w-1/2">
<form id="delete-form-{{ $item->id }}" action="{{ route('destinataires.destroy', ['destinataire'=>$item->id])}}" method="POST">
          @csrf
          @method("DELETE")
          <button type="submit">
      ...
      @can('modifier_client')
          <div class="w-1/2">
        <a href="{{ route('destinataires.edit', ['destinataire'=>$item->id])}}">
    ...
    <div class="mt-5">
    {{$destinataire->links("pagination::tailwind")}}
    </div>
  </div>
    </div>
