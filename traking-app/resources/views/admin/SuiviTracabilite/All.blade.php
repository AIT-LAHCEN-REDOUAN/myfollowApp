...
@can('afficher_operations')
    <h1 class="text-white text-center font-bold bg-gray-100 p-2 rounded" style="min-width: 100px!important;position: absolute;margin-top: 80px;margin-left: 30px;color:darkred">Traçabilité</h1>

    <br>
    <br>
    <br>
    <div class='flex flex-col overflow-x-auto overflow-y-auto p-8' style="margin-top: 80px!important">
<form action="{{route('operation_export')}}" method="get">
  <button style="background-color: #080f96"
    ...
    <div class="relative flex items-center mt-2">
        <div class="relative flex-grow">
          <span class="z-10 h-full leading-snug font-normal text-center text-blueGray-300 absolute bg-transparent rounded text-base items-center justify-center w-8 pl-3 py-3">
            ...
            >
                <option @if(isset($etat) && $etat == "In") selected @endif value="In">In</option>
                <option @if(isset($etat) && $etat == "Out") selected @endif value="Out">Out</option>
                <option @if(isset($etat) && $etat == "Retour") selected @endif value="Retour">Retour</option>
                <option @if(isset($etat) && $etat == "Pending") selected @endif value="Pending">Pending</option>
            </select>
        </div>
    </div>
{{--    <div class="relative flex items-center m-2">--}}
...
{{--    </div>--}}
  ...
  <button style="background-color: rgb(166,3,3);"
...
<br>
        ...
        <div class='flex flex-col overflow-x-auto overflow-y-auto'>
            <table id="myTable" style="background-color: white!important;" class='table-auto w-full'>
                <thead>
                <tr class='w-100 bg-red-600 text-white font-light'>
                    ...
                    <th class='px-4 py-2 text-center whitespace-nowrap'>Nom d'opérateur</th>
                    <th class='px-4 py-2 text-center whitespace-nowrap' style="width: 100%!important;">Quantité Avant Traitement</th>
                    <th class='px-4 py-2 text-center whitespace-nowrap' style="width: 100%!important;">Quantité Après Traitement</th>
                    <th class='px-4 py-2 text-center whitespace-nowrap' style="width: 100%!important;">Quantité Sortie</th>
                    <th class='px-4 py-2 text-center whitespace-nowrap'>Quantité Retour</th>
                    <th class='px-4 py-2 text-center whitespace-nowrap'>Statut</th>
                    <th class='px-4 py-2 text-center whitespace-nowrap'>Type De Mouvement</th>
                    <th class='px-4 py-2 text-center whitespace-nowrap'>Raison</th>
                    <th class='px-4 py-2 text-center whitespace-nowrap'>Motif</th>
                ...
                @foreach ($Suivi_de_tracabilite as $item)
                    <tr>
                        ...
                        <td class='px-4 py-2 text-center whitespace-nowrap'>{{$item->Fournisseur->name ?? "---"}}</td>
                        <td class='px-4 py-2 text-center whitespace-nowrap'>{{$item->destinataire->name ?? "---"}}</td>
                        <td class='px-4 py-2 text-center'>{{$item->colie->Prix ?? "---"}}</td>
                        <td class='px-4 py-2 text-center'>{{$item->user->name ?? "---"}}</td>
                        <td class='px-4 py-2 text-center whitespace-nowrap' style="width: 100%!important;">{{($item->etat!="Retour" && $item->etat!="Out")?($item->Quantite_avant_traitement ?? "---"):"---"}}</td>
                        <td class='px-4 py-2 text-center whitespace-nowrap' style="width: 100%!important;">{{($item->etat!="Retour" && $item->etat!="Out")?($item->Quantite_apres_traitement ?? "---"):"---"}}</td>
                        <td class='px-4 py-2 text-center whitespace-nowrap' style="width: 100%!important;">{{$item->Quantite_sortie ?? "---" }}</td>
                        <td class='px-4 py-2 text-center'>{{$item->Quantite_Retour ?? "---" }}</td>
                        <td class='px-4 py-2 text-center'>{{$item->statut ?? "---"}}</td>
                        <td class='px-4 py-2 text-center'>{{$item->etat ?? "---"}}</td>
                        <td class='px-4 py-2 text-center'>{{$item->raison ?? "---"}}</td>
                        <td class='px-4 py-2 text-center'>{{$item->motif ?? "---" }}</td>
