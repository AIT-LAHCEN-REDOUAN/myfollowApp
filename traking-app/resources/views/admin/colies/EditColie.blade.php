...
@can('modifier_produit')
    <h1 class="text-white text-center font-bold bg-gray-100 p-2 rounded" style="min-width: 100px!important;position: absolute;margin-top: 80px;margin-left: 30px;color:darkred">Produits / Modifier</h1>
    ...
    <div class="md:grid md:grid-cols-3 md:gap-6">
      <div class="mt-5 md:mt-0 md:col-span-2">
        ...
        <form enctype="multipart/form-data" action="{{route('colies.update',['coly'=>$item->Reference])}}" method="POST">
          @method("PUT")
          @csrf
          <div class="shadow overflow-hidden sm:rounded-md">
            <div class="px-4 py-5 bg-white sm:p-6">
              <div class="grid grid-cols-2 gap-6">
                  ...
                  <input readonly @required(true) value={{$item->Reference}}  type="text" name="Reference" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>
                <!-- Repeat this pair of input fields as needed -->
                <div class="col-span-2 sm:col-span-1">
                  <label  class="block text-sm font-medium text-gray-700">Désignation</label>
                  <input readonly @required(true) value={{$item->Designation}} type="text" name="Designation" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500  block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"/>
                </div>
                <div class="col-span-2 sm:col-span-1">
                  <label class="block text-sm font-medium text-gray-700">Prix Unitaire D'achat</label>
                  <input @required(true) value={{$item->Prix}} step="0.001"  type="number" min="0" name="Prix" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                </div>
                <!-- End of the repeated input fields -->
                <div class="col-span-2 sm:col-span-1">
                  <label class="block text-sm font-medium text-gray-700">Quantite Standard</label>
                  <input @required(true) value={{$item->Qte_Unitaire}} type="number" min="0" name="Quantite" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                </div>
                  <input style="display: none" @required(true) value="{{$item->id_Fournisseur}}" type="number"  name="Fournisseur" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                  <div class="col-span-2 sm:col-span-1">
                  <label  class="block text-sm font-medium text-gray-700">Fournisseur</label>
                    <input readonly @required(true) value="{{$item->Fournisseur->name}}" type="text"  name="" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    {{--                  <select @readonly(true) @required(true) name="Fournisseur" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">--}}
{{--                    @foreach ($fournisseur_name as $item)--}}
{{--                       <option>{{$item->name}}</option>--}}
{{--                    @endforeach--}}
{{--                    @foreach ($data as $items)--}}
{{--                    <option {{ $items->id==$item->id_Fournisseur ? 'selected' : '' }}  value={{$items->id}}>{{$items->name}}</option>--}}
{{--                    @endforeach--}}
{{--                  </select>--}}
                </div>
