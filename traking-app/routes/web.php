...
Route::get("/",function (Request $request){
    ...
    if ($request->input('start_date') && $request->input('end_date')) {
        ...
        $total_operation_pending = Suivi_de_tracabilites::where("statut","Pending")->whereBetween('created_at', [$startDateTime, $endDateTime])->get()->count();
        $total_operation_corrigee = Suivi_de_tracabilites::where("statut","Corrigée")->whereBetween('created_at', [$startDateTime, $endDateTime])->get()->count();
        ...
        $total_operation_annulee = Suivi_de_tracabilites::where("statut","Annuler")->whereBetween('created_at', [$startDateTime, $endDateTime])->get()->count();
    ...
    return view("admin.dashboard",["ca_by_months"=>$ca_by_months,"operations_by_month"=>$operations_by_month,"in_operations_par_statut"=>$in_operations_par_statut,"operations_par_etat"=>$operations_par_etat,"total_colie"=>$get_total_colie,"total_operation_derogee"=>$total_operation_derogee,"total_operation_annulee"=>$total_operation_annulee,"total_operation_corrigee"=>$total_operation_corrigee,"total_operation_pending"=>$total_operation_pending,"total_fournisseur"=>$get_total_fournisseur,"total_destinataire"=>$get_total_destinataire,"total_operation"=>$total_operation,"total_in"=>$total_produit_in,"total_out"=>$total_produit_out,"total_retour"=>$total_produit_retour,"ca"=>$ca]);
})->middleware("auth");
...
Route::resource("colies",ColieController::class)->middleware(["auth","permission:produits"]);
Route::resource("fournisseurs",FournisseurController::class)->middleware(["auth","permission:fournisseurs"]);
Route::resource("destinataires",destinataireController::class)->middleware(["auth","permission:destinataires"]);
Route::resource("users",UserController::class)->middleware(["auth","permission:users"]);


Route::get("/out",[SuiviTracabilite::class,"out"])->middleware(["auth","permission:afficher_operations_out"])->name("out");
Route::get("/in",[SuiviTracabilite::class,"in"])->middleware(["auth","permission:afficher_operations_in"])->name("in");
Route::get("/retour",[SuiviTracabilite::class,"retour"])->middleware(["auth","permission:afficher_operations_retour"])->name("retour");


Route::delete("/delete_in/{id}",[SuiviTracabilite::class,"delete_in"])->middleware(["auth","permission:supprimer_operations"])->name("delete_in");
Route::delete("/delete_out/{id}",[SuiviTracabilite::class,"delete_out"])->middleware(["auth","permission:supprimer_operations"])->name("delete_out");
Route::delete("/delete_retour/{id}",[SuiviTracabilite::class,"delete_retour"])->middleware(["auth","permission:supprimer_operations"])->name("delete_retour");
Route::delete("/delete_NonValider/{id}",[SuiviTracabilite::class,"delete_NonValider"])->middleware(["auth","permission:supprimer_operations"])->name("delete_NonValider");
Route::get("/all_operation",[SuiviTracabilite::class,"All"])->middleware(["auth","permission:afficher_operations"])->name("All");
...
Route::get("/NonValider",[SuiviTracabilite::class,"NonValider"])->middleware(["auth","permission:afficher_operations_nonValider"])->name("NonValider");

Route::get("/edit_operations/{id_op}",[SuiviTracabilite::class,"edit_operations"])->middleware(["auth","permission:modifier_operations"])->name("edit_op");
...
})->middleware(["auth","permission:ajouter_operateur"])->name("addOperateur");
...
})->middleware(["auth","permission:ajouter_operateur"])->name("AddOp");
...
Route::get("/Annuler",function (){
    ...
    $total_produit_retour= Suivi_de_tracabilites::where("etat","Retour")->count();
    $total_operation = Suivi_de_tracabilites::all()->count();
    $get_annuler = Suivi_de_tracabilites::where("Statut","Annuler")->where("etat","In")->orderBy("created_at","desc")->paginate(20);
...
Route::get("out/Annuler",function (){
    $get_out_annuler = Suivi_de_tracabilites::where("Statut","Annuler")->where("etat","Out")->with('destinataire')->orderBy("created_at","desc")->paginate(20);
    return view("admin.SuiviTracabilite.outAnnuler",["Suivi_de_tracabilite"=>$get_out_annuler]);
})->name("out.Annuler")->middleware("auth");

Route::get("Pending",function (){
    $get_pending = Suivi_de_tracabilites::where("Statut","Pending")->where("etat","In")->orderBy("created_at","desc")->paginate(20);
...
Route::get("out/Pending",function (){
    $get_out_pending = Suivi_de_tracabilites::where("Statut","Pending")->where("etat","Out")->orderBy("created_at","desc")->paginate(20);
...
Route::get("/Corrige",function (){
    ...
    $total_produit_retour= Suivi_de_tracabilites::where("etat","Retour")->count();
    $total_operation = Suivi_de_tracabilites::all()->count();
    $get_corriger = Suivi_de_tracabilites::where("Statut","Corrigée")->where("etat","In")->orderBy("created_at","desc")->paginate(20);
...
Route::get("out/Corrige",function (){
    $get_out_corriger = Suivi_de_tracabilites::where("Statut","Corrigée")->where("etat","Out")->with('destinataire')->orderBy("created_at","desc")->paginate(20);
...
Route::get("/Deroger",function(){
    ...
    $total_produit_retour= Suivi_de_tracabilites::where("etat","Retour")->count();
    $total_operation = Suivi_de_tracabilites::all()->count();
    $get_deroger = Suivi_de_tracabilites::where("Statut","Déroger")->where("etat","In")->orderBy("created_at","desc")->paginate(20);
...
Route::get("/AnnulerNonValider",function (){
    ...
    $total_produit_retour= Suivi_de_tracabilites::where("etat","Retour")->count();
    $total_operation = Suivi_de_tracabilites::all()->count();
    $get_AnnulerNonValider = Suivi_de_tracabilites::where("Statut", "Annuler")->where("etat","N/A")->orderBy("created_at", "desc")->paginate(20);
...
Route::get("/CorrigeNonValider",function (){
    ...
    $total_produit_retour= Suivi_de_tracabilites::where("etat","Retour")->count();
    $total_operation = Suivi_de_tracabilites::all()->count();
    $get_AnnulerNonValider = Suivi_de_tracabilites::where("Statut", "Corrigée")->where("etat","N/A")->orderBy("created_at", "desc")->paginate(20);
...
Route::get("/DerogerNonValider",function (){
    ...
    $total_produit_retour= Suivi_de_tracabilites::where("etat","Retour")->count();
    $total_operation = Suivi_de_tracabilites::all()->count();
    $get_AnnulerNonValider = Suivi_de_tracabilites::where("Statut", "Déroger")->where("etat","N/A")->orderBy("created_at", "desc")->paginate(20);
...
Route::resource("stocks",StockController::class)->middleware(["auth","permission:afficher_stock"]);
