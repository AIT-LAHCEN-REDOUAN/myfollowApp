...
@section('data')
@can('afficher_fournisseur')
    <h1 class="text-white text-center font-bold bg-gray-100 p-2 rounded" style="min-width: 100px!important;position: absolute;margin-top: 80px;margin-left: 30px;color:darkred">Fournisseurs</h1>
    ...
    <div class='flex flex-col overflow-x-auto overflow-y-auto p-8' style="margin-top: 80px!important">
        @if (session('success'))
            <div class="alert">
                Le fournisseur a été bien ajouter !
                <span class="closebtn text-xl " onclick="this.parentElement.style.display='none';">&times;</span>
            </div>
        @endif
<form action="{{route('export_fournisseur')}}" method="get">
  <button style="background-color: violet"
...
<div class="flex">
            @can('supprimer_fournisseur')
                <div class="w-1/2">
<form id="delete-form-{{ $item->id }}" action="{{ route('fournisseurs.destroy', ['fournisseur'=>$item->id])}}" method="POST">
          @csrf
          @method("DELETE")
          <button type="submit">
            <svg xmlns="http://www.w3.org/2000/svg" height="18" width="18" viewBox="0 0 448 512"><!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><style>svg{fill: #080f96
                    }</style><path d="M135.2 17.7L128 32H32C14.3 32 0 46.3 0 64S14.3 96 32 96H416c17.7 0 32-14.3 32-32s-14.3-32-32-32H320l-7.2-14.3C307.4 6.8 296.3 0 284.2 0H163.8c-12.1 0-23.2 6.8-28.6 17.7zM416 128H32L53.2 467c1.6 25.3 22.6 45 47.9 45H346.9c25.3 0 46.3-19.7 47.9-45L416 128z"/></svg>
          </button>
        </form>
                </div>
            @endcan

    @can('modifier_fournisseur')
        <div class="w-1/2">
        <a href="{{ route('fournisseurs.edit', ['fournisseur'=>$item->id])}}">
          <svg xmlns="http://www.w3.org/2000/svg" height="18" width="18" viewBox="0 0 512 512"><!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><style>svg{fill: #080f96
...
<div class="mt-5">
    {{$fournisseur->links("pagination::tailwind")}}
</div>
