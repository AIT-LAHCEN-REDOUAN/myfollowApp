...
class UserController extends Controller
{
    ...
    public function index()
    {
        ...
        $get_users=User::with("role")->orderBy("updated_at","desc")->paginate(10);
...
//        dd($request->all());
        ...
        $new_user = User::create([
            ...
            "Ville"=>strip_tags($request->input("Ville")),
    ...
    public function update(Request $request, $id)
    {
        ...
        $previousImage = $currentUser->Image;
        ...
        $affectedRows = DB::table('users')
            ...
            ->update([
                ...
                "original_password" => strip_tags($request->input("password")),
                'id_role' => strip_tags($request->role),
                'Image' => '', // Clear the previous image for now
            ]);

        // Delete the previous image
        if (!empty($previousImage) && File::exists(public_path($previousImage))) {
        ...
        if ($request->hasFile("Photo")) {
            $newImage = $request->file("Photo")->move("Images",$request->file("Photo")->getClientOriginalName());
